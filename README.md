...
TodoHubは、GitHubのIssue機能をバックエンドとして利用した、シンプルでクロスプラットフォームなTodo管理アプリケーションです。.NET MAUIで構築されています。
...
アプリを使用するには、GitHubのPersonal Access Token (PAT) が必要です。

1.  GitHubの設定 ([Developer settings](https://github.com/settings/tokens)) にアクセスします。
2.  **Personal access tokens** (Tokens (classic) 推奨) を生成します。
